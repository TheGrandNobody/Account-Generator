...
# Type the number of tokens you want, where 1 = 1 Token that has 18 decimals = 1 BEP20 token
AMOUNT_OF_TOKENS_MIN = 0.001
# The maximum amount of tokens you would like to send to any given account
# Type the number of tokens you want, where 1 = 1 Token that has 18 decimals = 1 BEP20 token
AMOUNT_OF_TOKENS_MAX = 0.002

# The address of the token you would like to send to each address
# Paste the BSC address of whatever token you want to send, in between the two quotation marks
TOKEN_ADDRESS = ''
        ...
        for i in range(NUMBER_OF_ACCOUNTS):
            ...
            if (SEND_TOKENS):
                print('Sent transaction number ', i, ', ',
                      NUMBER_OF_ACCOUNTS - (i + 1), ' transactions left')
                # Create a transaction
                transaction = token.functions.transfer(
                    acc.address,  # The address to which we send tokens to
                    # The amount of tokens being sent 1 = 1 Token that has 18 decimals = 1 BNB
                    w3.toWei(random.randint(AMOUNT_OF_TOKENS_MIN,
                             AMOUNT_OF_TOKENS_MAX), 'ether'),
                ).buildTransaction({
                    # If you change this to 97 it sets it to the Testnet. 56 is the Mainnet.
                    'chainId': 56,
                    'gas': 2000000,
